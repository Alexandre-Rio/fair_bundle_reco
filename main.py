...
def create_args():
    ...
    parser.add_argument('--problem_config', type=str, default='ml-100k_budget')
    ...
    parser.add_argument('--lambda_pfair_update', action='store_false')
...
def run(args):
    ...
    for t in range(args.problem_config["horizon"]):
        ...
        # Sample a budget
        budget = np.random.normal(loc=args.problem_config["budget_mean_multiplier"] * cost_vector.mean(), scale=cost_vector.std())
        budget = budget * (args.problem_config["budget_mean_multiplier"] > 0)
        ...
        if args.algo == "ilp":
            bundle, info = ilp(list_items=ps_idx,
                               ...
                               cost_vec=cost_vector.loc[ps_idx].to_numpy().squeeze(-1),
        ...
        if args.compute_optimal:  # Compute bundle without fairness
            ...
            opt_bundle_history.append((user_idx, budget, opt_bundle, opt_info, opt_quality, opt_relevance, opt_similarity, opt_bundle_cat, fairness_opt))
        ...
        if args.compute_optimal:
            bundle_history.append(
                (user_idx, budget, bundle, info, bundle_quality, bundle_relevance, bundle_similarity, bundle_cat,
    ...
    if args.compute_optimal:
        bundle_df.columns = ['user', 'budget', 'bundle', 'info', 'quality', 'relevance', 'similarity', 'categories',
        ...
        bundle_df.columns = ['user', 'budget', 'bundle', 'info', 'quality', 'relevance', 'similarity', 'categories', 'fairness_bound', 'fairness', 'size', 'lambda']
    bundle_df.to_csv(f'./runs/{run_name}/bundle_history.csv')
    if args.compute_optimal:
        opt_df = pd.DataFrame(opt_bundle_history)
        opt_df.columns = ['user', 'budget', 'bundle', 'info', 'quality', 'relevance', 'similarity', 'categories', 'fairness']
